...
class CT1Manager extends React.Component {
  ...
  handleUploadCTInfo = () => {
    this.uploadInfoForm.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        ...
        uploadCTInfoParams.append('ifTuberculosis', 0);
  ...
  render() {
    ...
    const columns = [
      ...
      //         <div>
      //             {markStatus ?
      //                 <Button type="primary" block>已处理</Button>
      //                 :
      //                 <Button type="primary" danger>未处理</Button>
      //             }
      //         </div>
      //     )
      // },
    ...
    const option = {
      ...
      legend: {
        bottom: 0,
        left: 'center',
        data: ['原发性肺结核', '血型播散性肺结核', '继发性肺结核', '正常']
      },
      series: [
        {
          type: 'pie',
          ...
          selectedMode: 'single',
          data: [
            { value: 15, name: '原发性肺结核' },
            { value: 11, name: '血型播散性肺结核' },
            { value: 68, name: '继发性肺结核' },
            { value: 6, name: '正常' },
          ],
          emphasis: {
            itemStyle: {
      ...
      <div>
        ...
        >
          <Form labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
            ...
            <Form.Item label="肺结核类别" name="diagnosisRecord">
              {getFieldDecorator('diagnosisRecord', {
                rules: [{ required: true, message: '请选择肺结核类别' }],
                initialValue: ctInfo.diagnosisRecord == null ? null : ctInfo.diagnosisRecord
              })(
                <Select style={{ width: 320 }} placeholder="肺结核类别">
                  <Option value="原发性肺结核">原发性肺结核</Option>
                  <Option value="血型播散性肺结核">血型播散性肺结核</Option>
                  <Option value="继发性肺结核">继发性肺结核</Option>
        ...
        >
          ...
          <div className="analysisContainer">
            <table border="1" className="analysisTable">
              ...
              <tbody>
                ...
                <tr>
                  <td>正常</td>
                  <td>{this.state.analysisTable[3]}</td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="analysisSelect">
            <Select placeholder="肺结核类别" value={checkedAnalysis ? checkedAnalysis.diagnosisRecord : '肺结核'} onChange={(value) => {this.setState({checkedAnalysis: {...checkedAnalysis, 'diagnosisRecord': value}})}}>
              <Option value="原发性肺结核">原发性肺结核</Option>
              <Option value="血型播散性肺结核">血型播散性肺结核</Option>
              <Option value="继发性肺结核">继发性肺结核</Option>
              <Option value="正常">正常</Option>
...
CT1Manager = Form.create({})(CT1Manager);
export default CT1Manager;